...
namespace fshafas.csharp
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            using (var client = new HafasAsyncClient(Db.getProfile()))
            {
                ...
                foreach (var l in locations)
                {
                    switch (l)
                    {
                        case U3<Client.Station, Client.Stop, Client.Location>.Case1 e:
                            Console.WriteLine($"Station {e.Item.name}");
                            break;
                        case U3<Client.Station, Client.Stop, Client.Location>.Case2 e:
                            Console.WriteLine($"Stop {e.Item.name}");
                            break;
                        case U3<Client.Station, Client.Stop, Client.Location>.Case3 e:
