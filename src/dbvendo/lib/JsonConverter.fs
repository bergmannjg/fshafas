...
module internal Converter =
    ...
    type U5EraseValueConverter<'a, 'b, 'c, 'd, 'e>(uc: UnionCaseSelection) =
        inherit JsonConverter<U5<'a, 'b, 'c, 'd, 'e>>()

        override this.Read(reader: byref<Utf8JsonReader>, _typ: Type, options: JsonSerializerOptions) =
            raise (System.NotImplementedException(""))

        override this.Write(writer: Utf8JsonWriter, value: U5<'a, 'b, 'c, 'd, 'e>, options: JsonSerializerOptions) =
            match value with
            | U5.Case1 v -> JsonSerializer.Serialize(writer, v, options)
            | U5.Case2 v -> JsonSerializer.Serialize(writer, v, options)
            | U5.Case3 v -> JsonSerializer.Serialize(writer, v, options)
            | U5.Case4 v -> JsonSerializer.Serialize(writer, v, options)
            | U5.Case5 v -> JsonSerializer.Serialize(writer, v, options)

    type U5EraseConverter<'a, 'b, 'c, 'd, 'e>(uc: UnionCaseSelection) =
        inherit JsonConverterFactory()

        override this.CanConvert(t: Type) : bool =
            t.IsGenericType
            && t.GetGenericTypeDefinition() = typedefof<U5<'a, 'b, 'c, 'd, 'e>>

        override this.CreateConverter(typeToConvert: Type, _options: JsonSerializerOptions) : JsonConverter =
            let types = typeToConvert.GetGenericArguments() |> Array.take 5

            let converterType =
                typedefof<U5EraseValueConverter<'a, 'b, 'c, 'd, 'e>>.MakeGenericType(types)
